...
.DESCRIPTION
    Reads exp6_variant<N>_<name>_<tag>.csv files from the experiment output
    directory (default: engine/out/exp6_puzzles_<latest>) and produces:
    ...
    Usage:
        ...
        .\run_exp6_analyze.ps1 -ExperimentDir engine\out\exp6_puzzles_combined
#>
...
# Auto-detect latest out/exp6_puzzles_* directory if not provided
if (-not $ExperimentDir) {
    $latestDir = Get-ChildItem -Path (Join-Path $engineDir 'out') -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -match '^exp6_puzzles_' } |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1

    if (-not $latestDir) {
        Write-Error "No exp6_puzzles_* directory found in out/. Run run_exp6_variant.ps1 first."
        exit 1
    }
...
$inputs = @(Get-ChildItem -Path $ExperimentDir -Filter $pattern -ErrorAction SilentlyContinue)
...
if ($inputs.Count -eq 0) {
    Write-Error "No exp6_variant*.csv files found in $ExperimentDir. Run run_exp6_variant.ps1 first."
    exit 1
}

$pyArgs = @($AnalysisScript, '--exp6-dir', $ExperimentDir)
